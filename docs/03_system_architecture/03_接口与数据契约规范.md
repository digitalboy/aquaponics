...
* `{component_id}`：L3/L4 设施或测点编号。如 `TK-01`（1号鱼池）、`BED-TP-01`（1号水培菜池）、`CB-HV-01`（强电柜）、`RFT-24H-A-0088`（浮板UID）。
...
| **设备指令控制** | `farm/SH01/HP/hydro/BED-TP-01/command/pump` | 种植子系统 $\rightarrow$ 执行器 |
| **设备心跳/遗嘱** | `farm/SH01/AQ/aqua/CB-LV-01/status/heartbeat` | 网关 $\rightarrow$ 运维中心 |
| **浮板调池事件** | `farm/SH01/HP/trace/RFT-24H-A-0088/event/transfer` | 扫码枪/AGV $\rightarrow$ 溯源中台 |
...
#### 驻厂品质实验室双维度批次检测与 e-COA 放行数据契约 (QualityLabBatchInspectionPayload)
```json
{
...
* **`safety_parameters` / `nutrition_parameters`**：驻厂检验室安全红线（硝酸盐/农残/抗生素/土腥味）与营养风味（维C/糖度Brix/粗蛋白）实测值与合格评定。
