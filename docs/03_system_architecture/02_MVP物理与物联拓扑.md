...
flowchart TD
    ...
    subgraph Layer3["3. IT 算力与服务层 (数据落湖)"]
        ...
        DB["云端/本地时序数据库<br>(InfluxDB / TDengine 存储)"]
    end
...
### 原理三：数据资产的“规范化与时空标签绑定”（网关 $\rightarrow$ 数据库）

* **过程：** 网关拿到原始数据并过滤噪点后，强制打包为标准 JSON 报文。报文中必须打上：
  1. 资产身份标识：`Batch_ID`（资产批次）、`Zone_ID`（车间区域）。
  2. 物理采集时间：**严格遵循 ISO 8601 的毫秒级 UTC 时间戳**（如 `2026-07-12T16:56:18.000Z`）。
  3. **流体时空对齐标签**（依据《接口与数据契约规范》，附加由流速算出的 `water_origin_timestamp` 和 `lag_seconds`）。
* **进入数据湖：** 最终，网关将带有高价值时空标签的 JSON 数据通过 MQTT 协议异步发布至总线，写入 InfluxDB/TDengine 时序数据库，供大后方的 AI 预测模型与 MPC 能耗控制进行分析套利。
