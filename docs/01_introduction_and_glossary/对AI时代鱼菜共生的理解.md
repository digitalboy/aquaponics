...
flowchart TD
    %% 样式定义
    classDef offline fill:#efebe9,stroke:#5d4037,stroke-width:1px,color:#111;
    classDef online fill:#e1f5fe,stroke:#0288d1,stroke-width:1px,color:#111;
    classDef storage fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px,color:#111;

    subgraph PlanA["方案 A: 离线数字集成"]
        direction LR
        A1["手工取样化验"]:::offline --> A2["实验室分光光度计"]:::offline
    end

    subgraph PlanB["方案 B: AI 软传感器反演"]
        direction LR
        B1["在线物理传感器<br>(pH / DO / Temp / EC)"]:::online --> B2["🧠 神经网络模型<br>(软传感器反演算法)"]:::online
    end

    Warehouse["📊 统一水质数仓"]:::storage

    A2 -->|LIMS / 标准 API| Warehouse
    B2 -->|实时反演数据| Warehouse
```
...
| **氨氮/亚硝** | 离线+AI反演 | 分光光度计 + 神经网络 | 1天/次 (人工)<br>5分钟/次 (AI) | LIMS系统 API 导入<br>gRPC 实时模型推理流 | 每日人工化验数据作为 AI 模型的在线校准校标（Ground Truth） |
...
| --- | --- | --- | --- | --- | --- |
| **鱼池水面** | IP66 工业大广角相机 | 投喂期间<br>(30帧/秒短视频) | **边缘网关 (Edge)**<br>本地实时推理 | 像素光流法 (Optical Flow)<br>+ 行为状态机 | 盲目过度投喂导致的水质崩坏与饲料浪费 |
| **鱼池水下** | IP68 工业双目相机<br>+ 蓝宝石视窗 | 24小时随机抽检时段 | **边缘计算一体机**<br>本地点云处理 | 三维骨架提取神经网络<br>+ 图像分割 (Mask R-CNN) | 人工捞鱼称重导致的鱼群应激死亡与数据滞后 |
| **温室高空** | 5000万像素 工业相机 | 每日正午 1 次定格 | **云端孪生层 (Cloud)**<br>异步批处理 | 语义分割 (SAM / U-Net)<br>+ 时间序列面积反演 | 作物生长速度无法量化，产销计划脱节 |
| **巡检轨道** | 轻量化工业多光谱相机 | 每日夜间/清晨巡检 | **边缘（初筛特征提取）**<br>+ **云端（NDVI热力图渲染）** | 辐射校准算法<br>+ 超像素异常检测 (Anomaly Detection) | 虫害爆发后才发现，导致大面积减产与农残超标 |

**总结：** 视觉传感布局，是鱼菜共生系统从“被动监测”跨越到“具身智能”的灵魂一步。我们用光学和算力替代了昂贵的化学试剂与繁重的人力劳动，让摄像头变成了透视生命体内部生理特征的“X光机”。当水下的鱼群动态与水上的叶片光谱化作源源不断的结构化特征流入我们的 AI 引擎时，这座农场在数字化层面上，才真正变成了一个完全透明、可绝对掌控的数字精密工业体。
...
flowchart TD
    %% 样式定义
    classDef main fill:#ede7f6,stroke:#5e35b1,stroke-width:2px,color:#111;
    classDef shield fill:#e8f5e9,stroke:#2e7d32,stroke-width:1px,color:#111;

    %% 节点定义
    Main["🛡️ 鱼菜共生三大 AI 预警防线"]:::main
    S1["📊 防线一：水质突变预警<br>(无监督孤立森林 / LSTM)"]:::shield
    S2["👁️ 防线二：生物视觉筛查<br>(YOLOv8 / 光流行为识别)"]:::shield
    S3["🔌 防线三：设备预测性维护<br>(变频器电流时频域小波变换)"]:::shield
...
* **数据逻辑与特征工程：** AI 引擎全时段订阅动力与环控回路电表的**变频器输出电流、有功功率、无功功率**，并匹配现场水流速、风机转速。
...
电费是设施农业最无情的利润杀手。传统的温室温控系统（如 PID 控制器）是极其被动的：室温高了才开风机，光照弱了才开补光灯。这种“见招拆招”的反馈机制，在面对具有巨大热惯性（Thermal Inertia）的数万吨水体与庞大玻璃温室时，会导致设备频繁在高功率状态下震荡运转，进而产生高额的阶梯电费。
...
flowchart LR
    %% 样式定义
    classDef input fill:#e8f5e9,stroke:#2e7d32,stroke-width:1px,color:#111;
    classDef engine fill:#ede7f6,stroke:#5e35b1,stroke-width:2px,color:#111;
    classDef output fill:#fff3e0,stroke:#ef6c00,stroke-width:1px,color:#111;

    %% 节点定义
    Weather["🌤️ 外部天气预报数据<br>(辐射 / 气温 / 云层)"]:::input
    History["📊 园区历史运行数据<br>(水体热惯性 / 设备 COP)"]:::input
    Sensors["📡 实时物联网传感器<br>(当前温湿度 / 水温 / 电表)"]:::input
    
    MPC["⚙️ MPC 算法中央引擎<br>(时段电价约束最小化)"]:::engine
    Execution["🔌 物理执行层<br>(仅执行第 1 步指令)"]:::output

    %% 连接关系
    Weather --> MPC
    History --> MPC
    Sensors --> MPC
    MPC -->|求解最优控制序列| Execution
```
...
flowchart TD
    %% 样式定义
    classDef device fill:#e1f5fe,stroke:#0288d1,stroke-width:1px,color:#111;
    classDef data fill:#efebe9,stroke:#5d4037,stroke-width:1px,color:#111;
    classDef model fill:#ede7f6,stroke:#5e35b1,stroke-width:2px,color:#111;
    classDef task fill:#e8f5e9,stroke:#2e7d32,stroke-width:1px,color:#111;

    %% 节点定义
    Camera["📷 工业级 3D 相机 (激光 iToF)"]:::device
    Collect["📥 采集原始空间几何数据"]:::data
    PointCloud["📊 实时三维高密点云 (Point Cloud)"]:::data
    Topology["🧠 语义拓扑图 (Semantic Topology)"]:::model
    
    Target["🎯 精准定位叶冠质心与切割点"]:::task
    Collision["🚧 构建障碍物与叶片碰撞体积"]:::task

    %% 连接关系
    Camera --> Collect
    Collect -->|千万级空间坐标| PointCloud
    PointCloud -->|无监督 3D 骨架提取| Topology
    Topology ---> Target
    Topology ---> Collision
```
...
flowchart TD
    %% 样式定义
    classDef main fill:#ede7f6,stroke:#5e35b1,stroke-width:2px,color:#111;
    classDef left fill:#e8f5e9,stroke:#2e7d32,stroke-width:1px,color:#111;
    classDef right fill:#e1f5fe,stroke:#0288d1,stroke-width:1px,color:#111;

    %% 节点定义
    Main["🤖 具身智能柔性收割双向指令流"]:::main

    subgraph LeftHand["🦾 左手：轻拿 (柔性力控闭环)"]
        direction TB
        L1["【左手：轻拿 ── 柔性力控闭环】<br>(气动软体抓手 Soft Gripper)"]:::left
        L2["[ 触觉阵列反馈: 压力控制在 0.2N 阈值 ]"]:::left
        L1 --> L2
    end

    subgraph RightHand["✂️ 右手：快切 (视觉导引精准切根)"]
        direction TB
        R1["【右手：快切 ── 视觉导引精准切根】<br>(微距图像伺服 Visual Servoing)"]:::right
        R2["[ 超声波/气动切刀: 精准剥离基质 ]"]:::right
        R1 --> R2
    end
